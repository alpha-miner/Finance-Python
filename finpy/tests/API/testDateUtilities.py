	...
	def testBizDatesList(self):
		...
		while currentDate <= self.toDate:
			...
			if cal.isBizDay(finpyDate):
				self.assertTrue(finpyDate.toDateTime() in bizDtList, "{0} is expected as a business day in {1}".format(finpyDate, cal))
	...
	def testHolDatesList(self):
		...
		while currentDate <= self.toDate:
			...
			if not cal.isBizDay(finpyDate):
				self.assertTrue(finpyDate.toDateTime() in holDtList, "{0} is expected as a holiday in {1}".format(finpyDate, cal))
			currentDate = (finpyDate + 1).toDateTime()
